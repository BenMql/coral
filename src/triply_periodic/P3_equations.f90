...
Module P3_equations
 ...
 type :: linear_operator_recipe_T   
    ...
    integer, allocatable :: kz_exponent (:)
 ...
 subroutine add_equation_to_zero_coupled_set(self)
   class(full_problem_recipe_T) :: self
 ...
 subroutine add_equation_to_kxky_coupled_set(self)
   class(full_problem_recipe_T) :: self
 ...
 subroutine add_coupled_zero_equations_to_full_recipe(self)
    ...
    allocate(self%zero_recipes(this_recipe)%mass%kz_exponent (0))
    ...
    allocate(self%zero_recipes(this_recipe)%stif%kz_exponent (0))
 ...
 subroutine add_coupled_kxky_equations_to_full_recipe(self)
    ...
    allocate(self%kxky_recipes(this_recipe)%mass%kz_exponent (0))
    ...
    allocate(self%kxky_recipes(this_recipe)%stif%kz_exponent (0))
 ...
 subroutine build_kxky_NL_term_in_full_recipe( self, atom_linear_ops)
  ...
  do iAtom = 1, size(atom_linear_ops)
   ...
   self%kxky_recipes(this_recipe)%NL%term(n_old+iAtom)%dz_exponent = atom_linear_ops(iAtom)%dz_exponent
 ...
 subroutine build_zero_sources_in_full_recipe( self, atom_linear_ops)
  ...
  do iAtom = 1, size(atom_linear_ops)
   ...
   self%sources%term(n_old+iAtom)%dz_exponent = atom_linear_ops(iAtom)%dz_exponent
 ...
 subroutine build_zero_NL_term_in_full_recipe( self, atom_linear_ops)
  ...
  do iAtom = 1, size(atom_linear_ops)
   ...
   self%zero_recipes(this_recipe)%NL%term(n_old+iAtom)%dz_exponent = atom_linear_ops(iAtom)%dz_exponent
 ...
 subroutine build_current_full_var_in_full_recipe( self, atom_linear_ops)
   ...
   do iAtom = 1, size(atom_linear_ops)
      ...
      buf_term(n_old+iAtom)%dz_exponent = atom_linear_ops(iAtom)%dz_exponent
 ...
 subroutine build_zero_stif_matrix_in_full_recipe( self, atom_linear_ops)
   ...
   allocate( buf_int( n_old + n_new_pieces))
   buf_int(1:n_old) = self%zero_recipes(this_recipe)%stif%kz_exponent
   call move_alloc(from=buf_int, to=self%zero_recipes(this_recipe)%stif%kz_exponent)
   ...
   do iAtom = 1, size(atom_linear_ops)
     ...
     self%zero_recipes(this_recipe)%stif%kz_exponent(n_old+iAtom) = atom_linear_ops(iAtom)%dz_exponent
 ...
 subroutine build_kxky_stif_matrix_in_full_recipe( self, atom_linear_ops)
   ...
   allocate( buf_int( n_old + n_new_pieces))
   buf_int(1:n_old) = self%kxky_recipes(this_recipe)%stif%kz_exponent
   call move_alloc(from=buf_int, to=self%kxky_recipes(this_recipe)%stif%kz_exponent)
   ...
   do iAtom = 1, size(atom_linear_ops)
     ...
     self%kxky_recipes(this_recipe)%stif%kz_exponent(n_old+iAtom) = atom_linear_ops(iAtom)%dz_exponent
 ...
 subroutine build_zero_mass_matrix_in_full_recipe( self, atom_linear_ops)
   ...
   allocate( buf_int( n_old + n_new_pieces))
   buf_int(1:n_old) = self%zero_recipes(this_recipe)%mass%kz_exponent
   call move_alloc(from=buf_int, to=self%zero_recipes(this_recipe)%mass%kz_exponent)
   ...
   do iAtom = 1, size(atom_linear_ops)
     ...
     self%zero_recipes(this_recipe)%mass%kz_exponent(n_old+iAtom) = atom_linear_ops(iAtom)%dz_exponent
 ...
 subroutine build_kxky_mass_matrix_in_full_recipe( self, atom_linear_ops)
   ...
   allocate( buf_int( n_old + n_new_pieces))
   buf_int(1:n_old) = self%kxky_recipes(this_recipe)%mass%kz_exponent
   call move_alloc(from=buf_int, to=self%kxky_recipes(this_recipe)%mass%kz_exponent)
   ...
   do iAtom = 1, size(atom_linear_ops)
     ...
     self%kxky_recipes(this_recipe)%mass%kz_exponent(n_old+iAtom) = atom_linear_ops(iAtom)%dz_exponent
 ...
 subroutine determine_kind_of_information(text_line, bstep, eofSig, stopSig)
    ...
    select case (bstep)
       ...
       ! from 'linear_variable_kxky', accepted values are 'linear_variable_{kxky,mean,full}'
       case (2)
