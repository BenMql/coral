   ...
   do iObj = 1, self%recipe%timeseries%numberOf_linearObjects( iVar )
      select case (self%recipe%timeseries%linear(iVar)%object(iObj)%kind)
          case ('volume')
                ...
                varAvg = varAvg / real(self%geometry%NZAA, kind=dp)
                !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                self%io_bookkeeping%dPosition = position_backup
                current_position = (timings%i_timestep-1)*8+1
   first_or_not = .False.
 ...
 subroutine import_quicksave(self)
   ...
   self%coupled_kxky_set(isys)%field(:,:,:,iVar) = self%linear_variables(1)%spec
   end do
   end do
 ...
 subroutine export_verticallyAvgedSlice(self, ivar, iTime, linear_or_quadra)
   ...
   select case (linear_or_quadra)
          ...
          local_tile = local_tile / real(self%geometry%NZAA, kind=dp)
