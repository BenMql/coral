...
mkdir ${WHERE_TO}/CheckPoints_BCP
mkdir ${WHERE_TO}/Restart
mkdir ${WHERE_TO}/Matrices
mkdir ${WHERE_TO}/Timeseries
mkdir ${WHERE_TO}/Snapshots 
        
ln -s ${CORAL_ROOT}/build/eady.exe ${WHERE_TO}/.
cp    ${CORAL_ROOT}/etc/python_scripts/periodic_box/*.py  ${WHERE_TO}/.
cp    ${CORAL_ROOT}/etc/input_files_examples/Eady_parameters.in  ${WHERE_TO}/.
cp    ${CORAL_ROOT}/etc/input_files_examples/Eady_saving_list.in  ${WHERE_TO}/saving_list.in
