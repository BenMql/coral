...
module P3_geometry
 ...
 subroutine Initialize_geometry(self)
   ...
   allocate (self%kz (self%NZAA          ))
   allocate (self%pz (self%NZAA          ))
   allocate (self%deAliase_x (self%spec%local_NX ))
   allocate (self%deAliase_y (self%spec%local_NY ))
   allocate (self%deAliase_z (self%NZAA          ))
   ...
   !!
   !> next, initialize the z-wavenumbers
   !!
   self%kz_box = 2._dp * pi / self%Lz
   kcut = self%kz_box * self%NZ / 2._dp
   do iz = 1, self%NZAA         
      k_index = iz
      !> array of wave numbers
      if (k_index.ge.(self%NZAA/2)) then
         self%kz (iz) =-real( &
                        (self%NZAA + 1 - k_index) * self%kz_box ,& 
                        kind=dp )
      else
         self%kz (iz) = real( (k_index-1) * self%kz_box ,& 
                        kind=dp )
      end if
      !> dealiasing (set the mode to zero if True)
      if ( abs(self%kz (iz)) .ge. kcut) then
         self%deAliase_z (iz) = .True.
      else 
         self%deAliase_z (iz) = .False.
      end if
   end do
   ...
   self%pz = Cmplx(0._dp, self%kz, kind=dp)
   if (my_rank.eq.0) then
   print*, '================================================================='
   print *, " Initializing geometry ... (P3_geometry.f90)"
 ...
 subroutine export_geometry_to_disk(self)
   ...
   Open (Unit=9, File=file_str, Status='replace', Access='stream')
   Write(9) self%Lx,     self%Ly,     self%Lz, &               
            self%kx_box, self%ky_box, self%kz_box, &
 ...
 subroutine read_input_file_parameters(geom)
   ...
   read(3,*) geom%Lz            ! 07 ! (DBL) gap (should=1.0_dp) ! 07 !
   read(3,*) empty_entry        ! .......... TIMESTEPPING ........... !
   read(3,*) misc_cargo%dt_max           ! 09 ! (DBL) maximum timestep    ! 09 !
   read(3,*) misc_cargo%tolerance_factor ! 10 ! (DBL) timestep tolerance  ! 10 !
   read(3,*) misc_cargo%CFL_C            ! 11 ! (DBL) CFL Factor          ! 11 !
   read(3,*) misc_cargo%scheme_id        ! 12 ! scheme                    ! 12 !
   read(3,*) empty_entry               ! ......... QUICKSAVE RESTART ....... !
   read(3,*) misc_cargo%qsave_restart  ! 14 ! (BOO) quicksave restart   ! 14 !
   read(3,*) empty_entry               ! ............ TIMER ................ !
   read(3,*) misc_cargo%time_limit     ! 16 ! (DBL) Time limit (in sec.)! 16 !
   close(3)      
