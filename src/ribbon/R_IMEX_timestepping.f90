 ...
 !============================================================================= 
 ! 
 ! MODULE: R_IMEX_timestepping
 !  
 !> @author 
 !> Ben Miquel, www.normalesup.org/~benmiquel, benmiquel.coral@tutanota.com 
 ! 
 ! DESCRIPTION 
...
module R_IMEX_timestepping
 ...
 type :: intermediate_step_buffer_2d_T !< intermediate 3D fields (cf ARS97)
   complex(kind=dp), allocatable :: K_hat(:,:) !< eq.(2.1a)
   complex(kind=dp), allocatable :: K_std(:,:) !< eq.(2.1b)
 end type intermediate_step_buffer_2d_T
 ...
 type :: coupled_set_2d_ops_vars_T
   complex(kind=dp), allocatable :: field(:,:)
   complex(kind=dp), allocatable :: rhs  (:,:)
   complex(kind=dp), allocatable :: aux  (:,:)
   type(zOperator_2d_1coupled_T) :: mass
   type(zOperator_2d_1coupled_T) :: stif
   type(zOperator_2d_1coupled_T) :: evol
   ! type(sest)
 ...
 end type coupled_set_2d_ops_vars_T
 ...
 subroutine differentiate_kxky ( self, field, dOrder)
   ...
   do ix = 1, domain_decomp% spec_iSize(2)
   do iz = 1, self%geometry%NZ - 1
   deriv(iz,iy,ix) = field(iz+1, ix)
   end do
   end do
   dummyPtr = C_loc(deriv(1,1))
   ...
   do ix = 1, domain_decomp% spec_iSize(2)
   field(self%geometry%NZ : domain_decomp% spec_iSize(1),ix) = cmplx(0._dp, 0._dp, kind=dp)
   end do
   end if
 end subroutine
 ...
 subroutine factorize_operators( self, dt_size, first_factor_bool )
   ...
   do iSys = 1, self%recipe%numberOf_coupled_kxky_systems
      ...
      ! set to the identity
      if  self%geometry%deAliase_x (i1) then  
         ...
         call wrap_zcsradd(self%coupled_kxky_set(iSys)%mass%csr(i1), &
                           self%coupled_kxky_set(iSys)%stif%csr(i1), &
                           aux_zCSR, zsca)
      end if
      call aux_zCSR%to_dia( self%coupled_kxky_set(iSys)%evol%dia(i1) ) 
      end do
      ! /////////////////////////////////////////////////////////
      ...
      if (self%geometry%this_core_has_zero_mode) then
         call aux_zCSR % identity( self%coupled_kxky_set(iSys)%shape%total   )
         call aux_zCSR % to_dia  ( self%coupled_kxky_set(iSys)%evol%dia(1) )
      end if
      ! /////////////////////////////////////////////////////////
 ...
 subroutine noisy_initial_conditions(self)
   ...
   do iSys = 1, self%recipe%numberOf_coupled_kxky_systems 
     ...
     zPhase = cmplx(cos(random_dScalar*2.*3.14159),&
                    ...
                    kind=dp)
     self%coupled_kxky_set(isys)%field(iz, ix) = &
 ...
 include "R_IMEX_timestepping_marching.f90"
 include 'R_IMEX_timestepping_output.f90'
 ...
 subroutine add_zsca_times_MIblock_to_kxky_operators (self, which, zsca, &
   ...
   do i1 = 1, self%mass%n1   
      select case (which)
             case ('mass')
             call zcsr_add_block_dcsr_z(self%mass%csr(i1), aux2, zsca(i1), &
                                        self%shape%equation_firstIndex(iEqn), &
                                        self%shape%variable_firstIndex(iVar))
             case ('stif')
             call zcsr_add_block_dcsr_z(self%stif%csr(i1), aux2, zsca(i1), &
                                        self%shape%equation_firstIndex(iEqn), &
                                        self%shape%variable_firstIndex(iVar))
             case default
             stop 'bad character argument in add_zsca_times_MIblock...'
 ...
 subroutine compute_systems_shapes(self)
   ...
   do iSys = 1, self%recipe%numberOf_coupled_kxky_systems
      ...
      if (Neqs.ne.NVars) Then              
         ...
         self%coupled_kxky_set(isys)%shape%spectral_local_NX = domain_decomp% spec_iSize(2)
         self%coupled_kxky_set(isys)%shape%physical_local_NX = domain_decomp% phys_iSize(2)
         ! on one hand...
         total_dof1 = Neqs * NZ - &
 ...
 subroutine build_all_matrices(self)
   ...
   do isys = 1, self%recipe%numberOf_coupled_kxky_systems
      ...
      if (self%geometry%this_core_has_zero_mode) then
       call self%coupled_kxky_set(iSys)%mass%csr(1) % empty(&
                            self%coupled_kxky_set(iSys)%shape%total )
       call self%coupled_kxky_set(iSys)%stif%csr(1) % identity(&
                            self%coupled_kxky_set(iSys)%shape%total )
      end if
...
pure function recipe_to_prefactors_array(a_recipe, iPiece, geo)
   ...
   integer :: i1
   allocate( sh (geo%spec%local_NX) )
   do i1 = 1, geo%spec%local_NX ! X is the slow index in spectral!
      sh(i1) = a_recipe%dsca(iPiece) &
                * ((geo%px(i1))**a_recipe%kx_exponent(iPiece))
   end do
...
end module R_IMEX_timestepping
