 ...
 subroutine one_step_beyond(self, my_dt)
   ...
   if  self%geometry%deAliase_x(ix) then  
       self%coupled_kxky_set(isys)%field(:,ix) = cmplx(0._dp, 0._dp, kind=dp)
   end if
   end do
   if (self%geometry%this_core_has_zero_mode) then
       self%coupled_kxky_set(isys)%field(:, 1) = cmplx(0._dp, 0._dp, kind=dp)
   end if
   end do
 ...
 subroutine LPIMEX_stif_aux_to_K_std(self, i)
   ...
   do ix = 1, self%geometry%spec%local_NX
   if  self%geometry%deAliase_x(ix) then  
       self%coupled_kxky_set(isys)%step(i)%K_std(:,ix) = cmplx(0._dp, 0._dp, kind=dp)
   end if
   end do
   if (self%geometry%this_core_has_zero_mode) then
       self%coupled_kxky_set(isys)%step(i)%K_std(:, 1) = cmplx(0._dp, 0._dp, kind=dp)
   end if
   end do
 end subroutine
 ...
 subroutine compute_cfl_based_timestep(self)
   ...
   w_over_dz_max = u_over_dx_max
   do iz = 1, self%geometry%phys%local_NZ
   w_over_dz_max = dMax1(w_over_dz_max,&
                         maxVal( dAbs( self%linear_variables(2)%phys(:,,iz) / &
...
!
   ...
   do iTerm = 1, self%recipe%kxky_recipes(iSys)%NL%n_pieces
      ...
      nl_buffer(:,ix) = self%recipe%kxky_recipes(iSys)%NL%term(iTerm)%dsca * &
   ...
   do ix = 1, domain_decomp% spec_iSize(2)
   if  self%geometry%deAliase_x(ix) then  
       self%coupled_kxky_set(isys)%step(i)%K_hat(:,ix) = cmplx(0._dp, 0._dp, kind=dp)
   end if
   end do
   if (self%geometry%this_core_has_zero_mode) then
       self%coupled_kxky_set(isys)%step(i)%K_hat(:, 1) = cmplx(0._dp, 0._dp, kind=dp)
   end if
   end do
   ...
   do iTerm = 1, self%recipe%zero_recipes(iSys)%NL%n_pieces
     nl_buffer_zero(:) = self%recipe%zero_recipes(iSys)%NL%term(iTerm)%dsca* &
       real(self%quadratic_variables(&
            self%recipe%zero_recipes(iSys)%NL%term(iTerm)%quad_var_index)%spec(:,1),&
            kind=dp)
   ...
   do iTerm = 1,  self%recipe%linear_vars_full( iVar )%n_terms
     select case (self%recipe%linear_vars_full( iVar )%Term(iTerm)%var_kind)
       case ('kxky')
          ...
          do ix = 1, self%geometry%spec%local_NX
          self%linear_variables(iVar)%spec(:,ix) = &
                       self%linear_variables(iVar)%spec(:,ix) + &
                       datBuffer(:,ix)*&
       ...
       case ('zero')
          ...
          self%linear_variables(iVar)%spec(:,1) =  self%linear_variables(iVar)%spec(:,1) &
 ...
 !subroutine horizontal_average_of_physical_quantity_inPlace(self, iVar)
   ...
   !local_a1 = sum(self%linear_variables(iVar)%phys, 1)
...
!
 ...
 subroutine add_source_dscalar(self, sourceIndex, definition)
   ...
   if (sourceIndex .gt. self%numberOf_sources) then
      print *, 'In R_user_sources_definitions.f90, the position of the'
